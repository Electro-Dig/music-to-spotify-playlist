...
把截图、公众号文章、YouTube DJ set、歌单截图、榜单、书摘和文字 tracklist 转成 Spotify 可匹配数据，并在用户确认后创建 Spotify 歌单的 Codex Skill。

默认语言：中文。English version is below.
...
```text
$music-source-to-playlist 帮我把这张截图里的歌做成 Spotify 歌单
```
...
3. **Publish & share**：先 dry-run，再按用户确认创建或追加歌单。

Obsidian/Markdown 整理是可选分支。只有当用户明确说要“保存笔记、归档、整理成文档、放进 Obsidian”时才做。
...
脚本默认只读取环境变量、`.env`，或用户显式传入的 `--credentials-note`。它不会自动扫描私人 Obsidian 笔记。
...
`music-source-to-playlist` is a Codex Skill that turns music screenshots, articles, YouTube DJ sets, playlist screenshots, charts, book excerpts, and text tracklists into Spotify-ready data and optional Spotify playlists.
...
- Creates or updates a playlist only after user confirmation.
- Optionally saves Obsidian/Markdown notes or generates posters.
...
- Browser OAuth authorization for playlist creation.

Spotify's current Web API docs say Web API use requires Premium. Development Mode apps also have allowlist limits. See `references/SPOTIFY_SETUP.md`, [Spotify Web API](https://developer.spotify.com/documentation/web-api), and [Quota modes](https://developer.spotify.com/documentation/web-api/concepts/quota-modes).

## Install

Place this folder in a supported skill root:
...
Do not commit `.env`, Client Secrets, access tokens, refresh tokens, token caches, private credential notes, or private chat screenshots.

The bundled script reads credentials only from environment variables, `.env`, or an explicitly supplied `--credentials-note`.
